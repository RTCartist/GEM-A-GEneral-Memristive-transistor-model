...
lb  = [-100, -100, -200, -200]; % Lower bounds [k_off_lb, k_on_lb, b_off_lb, b_on_lb];
ub  = [100, 100, 200, 200];     % Upper bounds [k_off_ub, k_on_ub, b_off_ub, b_on_ub];

% Define pulse sequence
% Example: 50 pulses of 0.2V with 1ms duration, followed by 20 pulses of -0.1V with 2ms duration
pulse_sequence = [-7.5 * ones(20, 1); 7.5 * ones(20, 1)];
pulse_duration = [1e-5 * ones(20, 1); 1e-5 * ones(20, 1)];
t_ref = [0; cumsum(pulse_duration)]; % Adding an initial 0 time point
v_ds  = 1; 
% Generate random current data after each pulse (for testing)
data = readmatrix('..\Simulated_Data\Reference18\switching.csv');
i_ref = data(:, 2);
...
function [v, i, x] = GEM_model(v_pulses, v_ds, t_vec, x_init, iv, input_parameters)
    ...
    % Ron = 504344.9316;
    Ron = 57012.54276;
    voff = -0.2; % Must be negative
    von = 0.2; % Must be positive
